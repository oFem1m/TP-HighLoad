...
| **Files**           | FileID (PK), UserID (FK), FileName, FileSize, FileHash, CurrentVersion, UploadDate, ParentFileID (FK)     | PostgreSQL   | FileID (PK), UserID (индекс)       | Горизонтальное шардирование по UserID                | Хранит метаданные файлов, включая версионность                                                     |
| **Chunks**          | ChunkID (PK), ChunkHash, ChunkSize, StoragePath, ReferenceCount, UploadDate                              | PostgreSQL   | ChunkHash (уникальный)             | Горизонтальное шардирование по ChunkHash             | Хранит уникальные чанки файлов, резервируется с помощью Postgres-репликации                        |
| **FileChunks**      | FileID (FK), ChunkID (FK), ChunkIndex                                                                    | Cassandra    | FileID + ChunkIndex (PK, кластерный ключ) | Горизонтальное шардирование по FileID                | Эффективное распределение данных для операций записи/чтения; репликация через стратегию RF=3 |
...
4. **Chunks**
   ...
   - **ReferenceCount**: Без индекса, так как это числовое поле обновляется часто.

5. **FileChunks**
   - **FileID + ChunkIndex**: Составной кластерный индекс для поиска и сортировки частей файла.
...
2. **Предрасчет ReferenceCount**
   - В таблице `Chunks` поле `ReferenceCount` обновляется заранее, чтобы минимизировать вычисления в реальном времени при удалении или добавлении файлов.
...
| **FileChunks**      | Cassandra        | Высокая нагрузка на запись/чтение, распределенная архитектура для обработки больших объемов данных. |
...
| **FileChunks**      | Горизонтальное по FileID                 | Cassandra: RF=3 (три реплики для отказоустойчивости).                          |
...
- Redis: `redis-py`  
- Cassandra: `Datastax Cassandra Driver`  
...
- **Cassandra:** снэпшоты через `nodetool snapshot`.  
...
| **Cassandra**                      | Хранение данных с высокой доступностью (для Chunks)    | Позволяет распределенно хранить данные, масштабируясь горизонтально, что снижает нагрузку на БД и повышает отказоустойчивость. |
...
- **Хранение файлов**: Загружаемые файлы сначала проходят через Chunking Service, который разбивает их на части и передает их в S3. Информация о частях сохраняется в Cassandra, чтобы обеспечить быстрый доступ к этим данным.
...
| **Chunking Service**      | **Горизонтальное масштабирование** через Kubernetes. Данные о частях файлов хранятся в Cassandra, настроенной на межрегиональную репликацию для обеспечения доступности и отказоустойчивости. |
...
| **Cassandra**             | **Межрегиональная репликация** с автоматическим распределением данных и поддержкой отказоустойчивости. Возможность восстановления данных из реплик в случае потерь. |
