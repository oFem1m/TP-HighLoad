...
### Таблица Files
  Хранит метаданные о файлах, загруженных пользователями в облако.
| Поле          | Тип данных  | Описание                                               |
|---------------|-------------|--------------------------------------------------------|
| `FileID`      | INT         | Уникальный идентификатор файла (PRIMARY KEY).           |
| `UserID`      | INT         | Идентификатор пользователя (FOREIGN KEY, Users).       |
| `FileName`    | VARCHAR     | Имя файла, загруженного пользователем.                 |
...
| `FileVersion` | INT         | Версия файла (используется для управления версиями).    |

### Таблица Metadata
  Хранит расширенные метаданные файлов, такие как хеши и тип файлов.
| Поле          | Тип данных  | Описание                                               |
|---------------|-------------|--------------------------------------------------------|
| `MetadataID`  | INT         | Уникальный идентификатор метаданных (PRIMARY KEY).      |
| `FileID`      | INT         | Идентификатор файла (FOREIGN KEY, Files).              |
| `FileType`    | VARCHAR     | Тип файла (например, `jpg`, `mp4`, `docx`).            |
| `FileHash`    | VARCHAR     | Хэш файла для проверки целостности данных.             |
| `Checksum`    | VARCHAR     | Контрольная сумма файла.                               |
| `EncryptionKey`| VARCHAR    | Ключ шифрования для защиты файла.                      |
...
### Таблица Backup
  Хранит информацию о резервных копиях данных пользователей.
| Поле          | Тип данных  | Описание                                               |
|---------------|-------------|--------------------------------------------------------|
| `BackupID`    | INT         | Уникальный идентификатор резервной копии (PRIMARY KEY). |
| `UserID`      | INT         | Идентификатор пользователя (FOREIGN KEY, Users).       |
| `BackupSize`  | BIGINT      | Размер резервной копии в байтах.                       |
| `BackupPath`  | VARCHAR     | Путь к сохраненным данным резервной копии.             |
| `BackupDate`  | TIMESTAMP   | Дата и время создания резервной копии.                 |
| `FileVersion` | INT         | Версия файла в резервной копии.                        |
...
### Таблица Cache
  Используется для хранения временных данных и ускорения доступа к часто запрашиваемой информации.
| Поле          | Тип данных  | Описание                                               |
|---------------|-------------|--------------------------------------------------------|
| `CacheID`     | INT         | Уникальный идентификатор кэша (PRIMARY KEY).            |
| `FileID`      | INT         | Идентификатор файла (FOREIGN KEY, Files).              |
| `CacheTime`   | TIMESTAMP   | Время создания кэша.                                   |
| `CacheData`   | BLOB        | Данные кэша (фрагменты файла или метаданные).          |
...
| **Metadata**      | В среднем 1–2 КБ на файл | 1,968 (доступ к файлам)       | 394 (обновление метаданных при удалении файлов) | **Eventual Consistency**. Для метаданных возможны небольшие задержки в синхронизации, однако в критических случаях — строгая консистентность. |
...
| **Backup**        | До нескольких ГБ на резервную копию | 40 (доступ к резервным копиям) | 20 (обновление резервных копий) | **Строгая консистентность**. Резервные копии должны быть строго консистентными для восстановления данных без ошибок. |
...
| **Cache**         | Зависит от кешируемых данных | 5,902 (синхронизация данных, кэшируется большинство операций) | Средняя (запись новых данных в кеш) | **Eventual Consistency**. Для кеша можно использовать eventual consistency, чтобы улучшить производительность за счет допустимых задержек. |
