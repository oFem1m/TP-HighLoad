...
1. **Функциональное разбиение по доменам**  
   Для iCloud балансировка нагрузки выполняется по доменам, связанным с конкретными функциями (например, хранение файлов, синхронизация, работа с почтой). Это позволяет разграничить ключевые функции по инфраструктуре и улучшить их производительность. Например:
   - `files.icloud.com` для работы с файлами,
   - `mail.icloud.com` для обработки электронной почты.
