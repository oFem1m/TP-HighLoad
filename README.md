...
| **Сервис**                 | **Основная функция**                                          | **CPU на контейнер** | **RAM на контейнер** | **Количество контейнеров** | **Сервера хостинга**           |
|----------------------------|-------------------------------------------------------------|-----------------------|-----------------------|----------------------------|---------------------------------|
| API Gateway                | Обработка запросов пользователей, маршрутизация             | 4 vCPU                | 8 GB                  | 150                        | Frontend сервера               |
| Сервис авторизации         | Аутентификация, управление сессиями                         | 6 vCPU                | 12 GB                 | 150                        | Сервера авторизации            |
| Обработка файлов           | Обработка данных пользователей                              | 8 vCPU                | 24 GB                 | 150                        | Хранилище файлов               |
| Кодирование мультимедиа    | Преобразование мультимедийных данных                        | 16 vCPU               | 64 GB                 | 150                        | Сервера обработки мультимедиа  |
| Синхронизация данных       | Синхронизация контактов, фотографий и других данных         | 6 vCPU                | 12 GB                 | 150                        | Сервера синхронизации          |
| Аналитика                  | Обработка логов, сбор и анализ метрик                       | 12 vCPU               | 48 GB                 | 150                        | Сервера аналитики              |
| DNS и балансировка         | DNS-серверы, балансировка нагрузки                          | 4 vCPU                | 8 GB                  | 50                         | DNS и балансировочные серверы  |

### Сервера и их конфигурация

| **Категория серверов**      | **Количество серверов** | **Аппаратная конфигурация**                                               | **Размещаемые сервисы**                                                    |
|-----------------------------|--------------------------|---------------------------------------------------------------------------|----------------------------------------------------------------------------|
| **Хранилище файлов**        | 15,000                  | 192 vCPU, 384 GB RAM, 1 PB SSD (подключение сотен SSD в RAID 10)          | Хранение данных пользователей                                             |
| **Обработка мультимедиа**   | 1,000                   | 192 vCPU, 768 GB RAM, 20 TB SSD                                          | Кодирование, преобразование мультимедийных данных                         |
| **Серверы аналитики**       | 500                     | 128 vCPU, 512 GB RAM, 20 TB SSD                                          | Обработка логов, сбор и анализ метрик                                      |
| **Буферные серверы**        | 400                     | 128 vCPU, 256 GB RAM, 20 TB SSD                                          | Резерв на случай роста нагрузки                                           |
...
4. **Отказоустойчивость**: При выходе из строя до 10% серверов или контейнеров система продолжает работать без каскадных сбоев.
