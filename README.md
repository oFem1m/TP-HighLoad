...
| Резервные копии       | 1 резервная копия                | 10 GB                | 10 GB                        | ~850,000 ТБ                                 |
...
RPS = 85,000,000 * 33.6 / 86,400 = 33,056
Пиковый RPS = 170,000,000 * 33.6 / 86,400 = 66,112

| Тип запроса                           | RPS      | RPS (Пиковый) |
|---------------------------------------|----------|---------------|
| Аутентификация                        | 10       | 20            |
| Аутентификация (по куке)              | 984      | 1968          |
| Загрузка файлов на облако             | 4919     | 9838          |
| Просмотр/скачивание файлов            | 9838     | 19676         |
| Удаление файлов                       | 1968     | 3936          |
| Создание/доступ к общим папкам/файлам | 492      | 984           |
| Синхронизация данных (контакты, фото) | 14,757   | 29514         |
...
| Загрузка файла (средний файл) | 102 400 КБ               | 0 КБ                      | Загрузка файлов (пример: 100 МБ). |
| Скачивание файла (средний файл) | 0 КБ                    | 102 400 КБ                | Скачивание файлов с сервера. |
| Просмотр/синхронизация данных | 50 КБ                   | 50 КБ                     | Обновление контактов, календарей и фотографий. |
| Резервное копирование (средний объем) | 512 000 КБ              | 0 КБ                      | Создание резервной копии данных устройства (~500 МБ). |
